...
func (m Model) runDiffFormatter(diffText string) (string, error) {
    ...
    cols := 0
    if m.state != nil { cols = m.state.Terminal.Cols }
    ...
    if cmdStr == "" { return diffText, nil }
    c := exec.Command("sh", "-lc", cmdStr)
    if cols > 0 { c.Env = append(os.Environ(), fmt.Sprintf("COLUMNS=%d", cols)) } else { c.Env = os.Environ() }
    c.Stdin = strings.NewReader(diffText)
    out, err := c.CombinedOutput()
    if err != nil { return diffText, err }
...
func configureVimKeyBindings(cfg *oviewer.Config) error {
    ...
    set("left", "h"); set("right", "l"); set("up", "k"); set("down", "j")
    set("top", "g"); set("bottom", "G"); set("search", "/"); set("exit", "q")
    set("page_down", " "); set("page_up", "b")
    return nil
}

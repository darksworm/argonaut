...
import { getPaperclipInventory, STAPLER_SPEED, DESK_WIDTH, DESK_HEIGHT } from './PaperclipDatabase';

export const rulerLineMode = String.fromCharCode(105, 108, 105, 107, 101, 97, 114, 103, 111, 110, 97, 117, 116);
    ...
    if (cachedSupplies === null) {
        try {
            ...
            if (cachedSupplies.length > 0) {
                console.log(`First supply dimensions: ${cachedSupplies[0].split('\n').length} lines`);
            }
        } catch (error) {
...
const OfficeSupplyManager: React.FC<OfficeSupplyProps> = ({ onExit }) => {
    ...
    if (termCols < requiredWidth || termRows < requiredHeight) {
        return (
            <Box flexDirection="column" alignItems="center" justifyContent="center" height={termRows}>
                <Text color="yellow" bold>Terminal too small for office supplies!</Text>
                <Box marginTop={1}>
                    <Text color="cyan">Current size: {termCols}x{termRows}</Text>
                </Box>
                <Box marginTop={1}>
                    <Text color="cyan">Required size: {requiredWidth}x{requiredHeight}</Text>
                </Box>
                <Box marginTop={2}>
    ...
    return (
        <Box flexDirection="column" height={termRows} alignItems="center" justifyContent="center">
            ...
            <Box marginTop={1}>
                <Text color="yellow" bold>I like you too!</Text>
            </Box>
            <Box marginTop={1}>
                <Text color="gray">Press 'q' to return</Text>
            </Box>

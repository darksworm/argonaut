...
export const App: React.FC = () => {
  ...
  const [scopeNamespaces, setScopeNamespaces] = useState<Set<string>>(
    new Set(),
  );
  ...
  useInput((input, key) => {
    ...
    if (mode === "search") {
      ...
      if (key.downArrow) {
        setSelectedIdx((s) =>
          Math.min(s + 1, Math.max(0, visibleItems.length - 1)),
        );
        return;
      }
      if (key.upArrow) {
  ...
  function toggleSelection() {
    ...
    if (view === "clusters") {
      ...
      const next = scopeNamespaces.has(val)
        ? new Set<string>()
        : new Set([val]);
  ...
  const visibleItems = useMemo(() => {
    ...
    if (view === "clusters") base = allClusters;
      ...
      base = filteredByNs.filter(
        (a) => !scopeProjects.size || scopeProjects.has(a.project || ""),
      );
  ...
  const tag =
    activeFilter && view === "apps" ? `<${view}:${activeFilter}>` : `<${view}>`;
    ...
    <Box flexDirection="column" paddingX={1} height={termRows - 1}>
      ...
      {mode === "search" && (
        <Box borderStyle="round" borderColor="yellow" paddingX={1}>
          ...
          <Box width={2} />
          <Text dimColor>
            (Enter {view === "apps" ? "keeps filter" : "opens first result"},
            Esc cancels)
          </Text>
        </Box>
      )}
        ...
        {mode === "help" ? (
          ...
          <Box flexDirection="column" flexGrow={1}>
            <ResourceStream
              ...
              appName={syncViewApp}
              appNamespace={
                apps.find((a) => a.name === syncViewApp)?.appNamespace
              }
              onExit={() => {
                setMode("normal");
                setResourcesApp(null);
              }}
            />
          </Box>
        ) : (

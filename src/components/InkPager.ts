...
export async function showInkPager(content: string, options: InkPagerOptions = {}): Promise<void> {
  ...
  const render = () => {
    ...
    for (let i = 0; i < maxRows; i++) {
      if (i < visibleLines.length) {
        ...
        if (options.searchEnabled !== false && searchTerm && line.toLowerCase().includes(searchTerm.toLowerCase())) {
    ...
    const controls = options.searchEnabled !== false 

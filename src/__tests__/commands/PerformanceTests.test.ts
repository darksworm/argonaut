...
class BenchmarkCommand implements Command {
  ...
  execute(context: CommandContext, iterations?: string): void {
    ...
    context.statusLog.info(`Completed ${count} operations in ${duration.toFixed(2)}ms`, "benchmark");
  }
}
...
class AsyncBenchmarkCommand implements Command {
  ...
  async execute(context: CommandContext, delay?: string): Promise<void> {
    ...
    context.statusLog.info(`Async operation took ${actualDuration.toFixed(2)}ms (expected ~${delayMs}ms)`, "async-benchmark");
  }
}
...
class MemoryIntensiveCommand implements Command {
  ...
  execute(context: CommandContext, size?: string): void {
    ...
    context.statusLog.info(`Created array of ${arraySize} items, memory delta: ${memoryDelta.toFixed(2)}MB`, "memory-test");
...
class ConcurrentCommand implements Command {
  ...
  async execute(context: CommandContext, delay?: string): Promise<void> {
    ...
    try {
      context.statusLog.info(`Instance ${instanceId} starting (${ConcurrentCommand.activeInstances} active)`, "concurrent");
...
class TimeoutTestCommand implements Command {
  ...
  async execute(context: CommandContext, timeout?: string): Promise<void> {
    ...
    try {
      ...
      context.statusLog.info("Operation completed within timeout", "timeout-test");
    } catch (error: any) {
...
class ResourceLeakCommand implements Command {
  ...
  execute(context: CommandContext): void {
    ...
    context.statusLog.info(`Allocated resource ${resource.id}, total: ${ResourceLeakCommand.resources.length}`, "resource-test");
...
class CPUIntensiveCommand implements Command {
  ...
  execute(context: CommandContext, complexity?: string): void {
    ...
    context.statusLog.info(`CPU intensive operation (${iterations} iterations) took ${duration.toFixed(2)}ms, result: ${result.toFixed(2)}`, "cpu-test");
  }
}
...
describe('Command Performance and Timeout Tests', () => {
  ...
  describe('memory performance tests', () => {
    it('should handle memory-intensive operations efficiently', () => {
      ...
      const initialMemory = typeof process !== 'undefined' ? 
  ...
  describe('timeout and cancellation tests', () => {
    it('should handle operation timeouts correctly', async () => {
      ...
      // Act & Assert
      await expect(command.execute(context, '50'))
    ...
    it('should handle timeout edge cases', async () => {
      ...
      try {
        await command.execute(context);
        expect(mockStatusLog.info).toHaveBeenCalledWith('Result: completed', 'timeout-edge');
      } catch (error: any) {
        expect(error.message).toBe('timeout');
        expect(mockStatusLog.error).toHaveBeenCalledWith('timeout', 'timeout-edge');
      }
    });
  });

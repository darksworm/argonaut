...
class MockCommandRegistry {
  ...
  async execute(commandLine: string, context: CommandContext): Promise<boolean> {
    ...
    if (command.canExecute && !command.canExecute(context)) {
      context.statusLog.warn(`Command ${commandName} cannot be executed in current state`, "command");
      return false;
    }
...
class MockNavigationCommand implements Command {
  ...
  constructor(private targetView: string, aliases: string[] = []) {
  ...
  execute(context: CommandContext): void {
    context.dispatch({ type: "RESET_NAVIGATION", payload: { view: this.targetView as any } });
  }
}
...
class MockAsyncCommand implements Command {
  ...
  async execute(context: CommandContext, delay?: string): Promise<void> {
    ...
    context.statusLog.info(`Starting async operation with ${delayMs}ms delay`, "async");
...
describe('Command Integration Tests', () => {
  ...
  describe('basic command execution', () => {
    ...
    it('should handle unknown commands gracefully', async () => {
      ...
      expect(mockStatusLog.error).toHaveBeenCalledWith("Unknown command: unknown-command", "command");
    });
  ...
  describe('command execution conditions', () => {
    ...
    it('should execute when conditions are met', async () => {
      ...
      context.state.server = { config: { baseUrl: 'https://test.com' }, token: 'test' };
  ...
  describe('performance and scalability', () => {
    ...
    it('should handle command arguments parsing efficiently', async () => {
      // Arrange
      const longArguments = Array.from({ length: 100 }, (_, i) => `arg${i}`).join(' ');
    ...
    it('should track command execution metrics', async () => {
      ...
      const instrumentedRegistry = new class extends MockCommandRegistry {
        async execute(commandLine: string, context: CommandContext): Promise<boolean> {
          ...
          executionCounts.set(commandName, (executionCounts.get(commandName) || 0) + 1);

...
function removeServiceAccountDefaults(obj: any): void {
    ...
    if (obj.spec.volumes && Array.isArray(obj.spec.volumes)) {
        obj.spec.volumes = obj.spec.volumes.filter((volume: any) =>
...
function removeCommonDefaults(obj: any): void {
    ...
    if (kind === 'Deployment') {
        ...
        if (obj.spec.strategy?.type === 'RollingUpdate' &&
 ...
 */
export function neatManifest(manifest: any, config: NeatConfig = DEFAULT_NEAT_CONFIG): any {
    ...
    if (cleaned.metadata) {
        ...
        Object.keys(essentialMetadata).forEach(key => {
            if (essentialMetadata[key as keyof typeof essentialMetadata] === undefined ||
                essentialMetadata[key as keyof typeof essentialMetadata] === null ||
                (typeof essentialMetadata[key as keyof typeof essentialMetadata] === 'object' &&
                    Object.keys(essentialMetadata[key as keyof typeof essentialMetadata] || {}).length === 0)) {
...
export function createNeatConfig(overrides: Partial<NeatConfig>): NeatConfig {
    return {
        ...
        metadataFields: [...DEFAULT_NEAT_CONFIG.metadataFields, ...(overrides.metadataFields || [])],
        metadataAnnotations: [...DEFAULT_NEAT_CONFIG.metadataAnnotations, ...(overrides.metadataAnnotations || [])],
        specFields: [...DEFAULT_NEAT_CONFIG.specFields, ...(overrides.specFields || [])],
        containerFields: [...DEFAULT_NEAT_CONFIG.containerFields, ...(overrides.containerFields || [])],
        serviceAccountFields: [...DEFAULT_NEAT_CONFIG.serviceAccountFields, ...(overrides.serviceAccountFields || [])]
    };
}

...
interface TailProcess {
  ...
  sessionId: string;
  filePath: string;
  watchingForNewSessions?: boolean;
}
...
export class LogTailer {
        ...
        return this.spawnTailProcess(sessionId, filePath, { follow, lines })
          .map(tailProcess => {
            ...
            if (session === 'latest' && follow) {
              this.startSessionWatching(tailProcess);
            }
            return tailProcess;
          });
      });
  ...
  // Stop the current tail process
  stop(): ResultAsync<void, { message: string }> {
    if (!this.tailProcess) {
      return err({ message: 'No log tailing process is running' });
    }

    // Stop session watching
    ...
    return ResultAsync.fromPromise(
      new Promise<void>((resolve, reject) => {
        ...
        const process = this.tailProcess.process;
      ...
      (error: any) => ({ message: error?.message || 'Failed to stop log tailing' })
    );
  }

  // Get the current tail process info
  getCurrentTail(): TailProcess | null {
  ...
  private startSessionWatching(tailProcess: TailProcess): void {
    if (this.sessionWatchInterval) {
      return; // Already watching
    }

    tailProcess.watchingForNewSessions = true;
    const currentSessionId = tailProcess.sessionId;
    ...
    this.sessionWatchInterval = setInterval(async () => {
      try {
        ...
        if (latestSessionId !== currentSessionId && existsSync(latestPath)) {
          ...
          }, 500); // Give old process time to die
  ...
  private stopSessionWatching(): void {
    ...
    if (this.tailProcess) {
      this.tailProcess.watchingForNewSessions = false;
    }
  }
}

// Singleton tailer instance
let globalTailer: LogTailer | null = null;

// Get or create the global log tailer
export function getLogTailer(): LogTailer {
  if (!globalTailer) {
    globalTailer = new LogTailer();
  }
  return globalTailer;
}

// Convenience function for quick tailing
export function tailLogs(options: TailOptions = {}): ResultAsync<TailProcess, { message: string }> {
  return getLogTailer().start(options);
}

// Convenience function to stop tailing
export function stopTailing(): ResultAsync<void, { message: string }> {
  return getLogTailer().stop();
}
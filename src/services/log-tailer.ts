...
export function tailLogs(options: TailOptions = {}): ResultAsync<void, { message: string }> {
  const { session = 'latest' } = options;

  return resolveSessionFile(session)
    .andThen(({ filePath }) => {
...
function resolveSessionFile(session: string): ResultAsync<{ sessionId: string, filePath: string }, { message: string }> {
  if (session === 'latest') {

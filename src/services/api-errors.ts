...
export function translateApiError(error: any): ApiError {
  ...
  if (error?.message?.includes('ECONNREFUSED') || error?.message?.includes('ENOTFOUND')) {
    return {
      message: 'Cannot connect to ArgoCD server. Please check if the server is running and accessible.',
      type: 'network',
      originalError: error
    };
  }
  ...
  if (error?.message?.includes('timeout') || error?.message?.includes('ETIMEDOUT')) {
    return {
      message: 'Request timed out. The ArgoCD server may be slow or unreachable.',
      type: 'network',
      originalError: error
    };
  }
  ...
  if (error?.status) {
    ...
    switch (status) {
      case 401:
        return {
          message: 'Authentication failed. Please run "argocd login" to authenticate.',
          code: status,
          type: 'auth',
          originalError: error
        };
      ...
      case 404:
        return {
          message: 'Resource not found. It may have been deleted or you may not have access to it.',
          code: status,
          type: 'not-found',
          originalError: error
        };
      ...
      case 409:
        return {
          message: argoError?.error || 'Resource conflict. The resource may have been modified by someone else.',
          code: status,
          type: 'server',
          originalError: error
        };
      ...
      case 422:
        return {
          message: argoError?.error || 'Invalid request data. Please check your input.',
          code: status,
          type: 'server',
          originalError: error
        };
      ...
      case 504:
        return {
          message: 'ArgoCD server error. The server may be experiencing issues.',
          code: status,
          type: 'server',
          originalError: error
        };
...
export async function wrapApiCall<T>(apiCall: () => Promise<T>): Promise<Result<T, ApiError>> {
  try {

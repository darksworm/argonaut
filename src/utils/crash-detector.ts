...
export async function runWithCrashDetection(options: CrashDetectorOptions): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', async (code, signal) => {
      ...
      const crashed = code !== 0 || (signal && !['SIGINT', 'SIGTERM'].includes(signal));
      
      if (crashed) {
        console.error(`💥 Process crashed with ${code !== null ? `code ${code}` : `signal ${signal}`}`);
        ...
        reject(new Error(`Process crashed with ${code !== null ? `code ${code}` : `signal ${signal}`}`));
      } else {
        resolve();
      }
    });

...
export async function getApplication(server: Server, name: string, appNamespace?: string, signal?: AbortSignal): Promise<ArgoApplication> {
  const params = new URLSearchParams();
  if (appNamespace) params.set('appNamespace', appNamespace);
  const data: any = await api(server, `/api/v1/applications/${encodeURIComponent(name)}${params.toString() ? `?${params.toString()}` : ''}`, { signal } as RequestInit).catch(() => null as any);
  return (data || {}) as ArgoApplication;
}

export async function getRevisionMetadata(server: Server, name: string, revision: string, appNamespace?: string, signal?: AbortSignal): Promise<RevisionMetadata | null> {
  const params = new URLSearchParams();
  if (appNamespace) params.set('appNamespace', appNamespace);
...
export async function getManifests(server: Server, name: string, revision?: string, signal?: AbortSignal, appNamespace?: string): Promise<string[]> {
  const params = new URLSearchParams();
...
export async function postRollback(server: Server, name: string, body: { id: number; name: string; dryRun?: boolean; prune?: boolean; appNamespace?: string }): Promise<any> {
  const params = new URLSearchParams();

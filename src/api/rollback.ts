...
export async function getApplication(baseUrl: string, token: string, name: string, appNamespace?: string, signal?: AbortSignal): Promise<ArgoApplication> {
  const params = new URLSearchParams();
  if (appNamespace) params.set('appNamespace', appNamespace);
  const data: any = await api(baseUrl, token, `/api/v1/applications/${encodeURIComponent(name)}${params.toString() ? `?${params.toString()}` : ''}`, { signal } as RequestInit).catch(() => null as any);
  return (data || {}) as ArgoApplication;
}

export async function getRevisionMetadata(baseUrl: string, token: string, name: string, revision: string, appNamespace?: string, signal?: AbortSignal): Promise<RevisionMetadata | null> {
  const params = new URLSearchParams();
  if (appNamespace) params.set('appNamespace', appNamespace);
...
export async function getManifests(baseUrl: string, token: string, name: string, revision?: string, signal?: AbortSignal, appNamespace?: string): Promise<string[]> {
  const params = new URLSearchParams();
...
export async function postRollback(baseUrl: string, token: string, name: string, body: { id: number; name: string; dryRun?: boolean; prune?: boolean; appNamespace?: string }): Promise<any> {
  const params = new URLSearchParams();
  ...
  return api(baseUrl, token, path, { method: 'POST', body: JSON.stringify(body) });
}
